...
| A7 | Configured checkpoint_interval, validation_interval, cpu_thread_limit, memory_ceiling_gb, resume_preference are recorded but not enforced by the worker | `aeon/job/worker.py:73–207` — the worker reads only `tcfg["ckpt_every"]`, `tcfg["log_every"]`, `tcfg["max_steps"]`, `tcfg["batch_size"]`, `tcfg["seed"]`, `tcfg["grad_clip"]`, `tcfg.get("aux_gate_penalty")`, `tcfg.get("resume")`, and `tcfg.get("sample_every")`. No enforcement of cpu-thread-limit, memory-ceiling, or the launcher-side validation_interval | CORRECTED (W10-9) | `test_worker_ignores_gui_settings` (flipped) + `tests/test_w10_9_desktop_operations.py::test_worker_reads_cpu_thread_limit_from_compute_policy`, `::test_worker_reads_memory_ceiling_and_safe_stops`, `::test_worker_runs_periodic_validation`, `::test_job_dataclass_has_compute_policy_field`, `::test_load_job_backfills_compute_policy_default` | W10-9 |
| A8 | Runtime throughput metrics are hard-coded zero placeholders | `aeon/job/worker.py:191` (`step_time_s=0.0, tokens_per_s_raw=0.0, useful_tokens_per_s=0.0`) | CORRECTED (W10-9) | `test_worker_emits_zero_placeholder_metrics` (flipped) + `tests/test_w10_9_desktop_operations.py::test_worker_measures_step_time_via_perf_counter`, `::test_worker_emits_tokens_per_second_derived_from_batch_size`, `::test_worker_actually_emits_nonzero_metrics_end_to_end` | W10-9 |
...
| A17 | Frozen preflight can return READY without a usable tokenizer or corpus | `aeon/config/preflight.py` — none of the 17 checks require a tokenizer file to load or a corpus to open. Missing tokenizer/corpus paths generate WARN, not BLOCKED. | CORRECTED (W10-8) | `test_preflight_does_not_block_on_missing_tokenizer_or_corpus` (flipped) + `tests/test_w10_8_fail_closed_preflight.py::test_frozen_mode_blocks_when_no_tokenizer_configured`, `::test_frozen_mode_blocks_when_no_corpus_configured`, `::test_frozen_mode_blocks_when_corpus_yields_zero_records`, `::test_frozen_mode_blocks_when_tokenizer_missing_on_disk`, `::test_frozen_mode_blocks_when_corpus_missing_on_disk`, `::test_frozen_mode_passes_when_tokenizer_and_corpus_are_real`, `::test_source_mode_still_warns_when_no_tokenizer_configured` | W10-8 |
| A18 | GUI Validate is a modal message box, no validation runs | `aeon/launcher/gui.py:344–347` — `_on_validate` only calls `messagebox.showinfo`; it does not construct a bounded validator or run one. | CORRECTED (W10-9) | `test_gui_validate_is_placeholder` (flipped) + `tests/test_w10_9_desktop_operations.py::test_on_validate_captures_subprocess_output`, `::test_on_validate_shows_scrollable_report` | W10-9 |
| A19 | GUI Recovery instructs the user to open a terminal | `aeon/launcher/gui.py:359–362` — `_on_recovery` displays a message asking the user to launch `Aeon.exe --recover <request.json>` themselves | CORRECTED (W10-9) | `test_gui_recovery_requires_terminal` (flipped) + `tests/test_w10_9_desktop_operations.py::test_on_recovery_enumerates_and_builds_decision_in_process` | W10-9 |
| A20 | GUI Diagnose discards subprocess output | `aeon/launcher/gui.py:354–355` — `subprocess.Popen([..., "--diagnose", ck], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)` | CORRECTED (W10-9) | `test_gui_diagnose_discards_output` (flipped) + `tests/test_w10_9_desktop_operations.py::test_on_diagnose_captures_output_not_devnull` | W10-9 |
