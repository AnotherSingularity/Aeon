...
def iter_batches(vocab_size, seq_len, batch_size, device, generator):
    """Synthetic random-token batches (pipeline smoke). Replace with a real
    tokenized corpus stream for an actual training run."""
    while True:
...
def main():
    ...
    batches = iter_batches(tcfg_model.vocab_size, dcfg["seq_len"],
